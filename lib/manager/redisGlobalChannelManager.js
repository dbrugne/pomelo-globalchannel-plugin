...
GlobalChannelManager.prototype.getChannelsKeysByChannelsName = function(servers, channelsName) {
  var channels = {};
  for(var sid in servers) {
    channels[sid] = [];
    for(var cid in channelsName){
      channels[sid].push(genKey(this, channelsName[cid], servers[sid]));
    }
  }
  return channels;
};
 ...
 * @param {Object}    channels  List of channels for each servers
 * @param {String}    uid       user id
 * @param {Function}  cb        callback function
 *
 * @return {Object}   users     List of members in channel sort by servers
 */
GlobalChannelManager.prototype.getMembersByMultiChannel = function(serversId, channels, uid, cb) {
  ...
  for(var sid in serversId) {
    if (channels[sid] && channels[sid].length) {
